...
import { getContactOptions, getEmployeeOptions, getMinuteRate } from "@/lib/queries";
...
export default async function NewOrder({ searchParams }: { searchParams: Promise<{ for_contact?: string }> }) {
  ...
  const [contacts, employees, minuteRate] = await Promise.all([getContactOptions(), getEmployeeOptions(), getMinuteRate()]);
